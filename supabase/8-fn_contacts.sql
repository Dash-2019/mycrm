CREATE TABLE audit_log_contact (
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        -- INSERT操作の新しいデータを記録
        INSERT INTO audit_log_contact (operation, new_data, query)
        VALUES ('INSERT', row_to_json(NEW), current_query());
    ELSIF TG_OP = 'UPDATE' THEN
        -- UPDATE操作の変更前と変更後のデータを記録
        INSERT INTO audit_log_contact (operation, old_data, new_data, query)
        VALUES ('UPDATE', row_to_json(OLD), row_to_json(NEW), current_query());
    ELSIF TG_OP = 'DELETE' THEN
        -- DELETE操作の削除されたデータを記録
        INSERT INTO audit_log_contact (operation, old_data, query)
...
CREATE TRIGGER audit_trigger_contact
